...
const maxInArray = (arr) => {
  return arr.reduce((maximum , number) =>{
    return number > maximum ? maximum = number : maximum = maximum;
  },0);
};
...
const hasChildrenValues = (arr, character) => {
  let array = Object.entries(arr.filter(element => element.name.includes(character))[0]);
  let value = array.filter(element => element.includes('children'));
