...
const count = (target, input) => {
  ...
  input.forEach(element => {
    Value = Value + element.reduce((acc, val) => {
      if (val === target) acc++;
      return acc;
    }, 0);
  });
...
const totalSum = (input) => {
  return input.reduce((final, item) => final = item.reduce((acc, Value) => final = final + Value, 0), 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(Value => Value.filter(Value => typeof Value === 'number' && Value % 5 === 0 ? true : false).map(number => Math.pow(2, number)));
};
...
let findMaleAndFemale = (data) => {
  ...
  }).reduce((acc,val)=>{ return acc + `${val.name} and `; },'').slice(0,-5);
};
...
let findShortest = (data) => {
  let shortest = data.reduce((acc, item) => item.height === data.reduce((acc, item) => acc < item.height ? acc = item.height : acc, 0) ? acc = item.name : acc, '');
  return shortest;
};
