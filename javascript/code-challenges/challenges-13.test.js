...
const findHappiness = (arr) => {
  return arr.filter(Value => Value.includes(':)') ? true : false);
};
...
const standardizePhoneNumbers = (arr) => {
  return arr.map(Value => `${Value.substring(1, 4)}${Value.substring(6, Value.length).split('-').join('')}`);
};
...
const allHappy = (arr) => {
  let New = false;
  arr.length === 0 ? New = true : New = arr.reduce((acc, Value) => Value.includes(':)') === false ? acc = false : acc , true);
  return New;
};
